...
#if defined(NDEBUG)
#error "Jadepay cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nBits, int nHeight) {
    	int64_t nSubsidy = 0;
	double dDiff =  ConvertBitsToDouble(nBits);

    if (nHeight == 0) {
        nSubsidy = 400000 * COIN;
    } else if (nHeight <= 480) { // 480 BLOCK, 1DAY
        nSubsidy = (1111.0 / (pow((dDiff+1.0),2.0))); // CPU/MOBILE difficulty calc
        if (nSubsidy > 5) nSubsidy = 5 * COIN;
	if (nSubsidy < 1) nSubsidy = 1 * COIN;
    } else if (nHeight <= 172800 && nHeight > 480) { // 1 YEAR
	nSubsidy = (11111.0 / (pow((dDiff+51.0)/6.0,2.0))); // CPU/MOBILE difficulty calc
        if (nSubsidy > 10) nSubsidy =  10 * COIN;
	if (nSubsidy < 1) nSubsidy = 1 * COIN;
    } else if (nHeight > 172800) { // 2 YEAR ++
        nSubsidy = (2222222.0 / (pow((dDiff+2600.0)/9.0,2.0))); // GPU/ASIC difficulty calc
        if (nSubsidy > 8) nSubsidy = 8 * COIN;
	if (nSubsidy < 1) nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount) {
    ...
    if (nHeight <= 480) { // 1 DAY 50%
        ret = 0;
    } else if (nHeight <= 172800 && nHeight > 480) { // 1 YEAR 60%
        ret = blockValue * .60;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 172800) { // 2 YEAR ++ 50%
        ret = blockValue  * .50;
    } else if (nHeight > Params().LAST_POW_BLOCK()) {
        int64_t nMoneySupply = chainActive.Tip()->nMoneySupply;
        int64_t mNodeCoins = mnodeman.size() * 20000 * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck) {
    ...
    if (!IsInitialBlockDownload() && !IsBlockValueValid(block, GetBlockValue(pindex->pprev->nBits, pindex->pprev->nHeight) + nFees)) {
        return state.DoS(100,
                error("ConnectBlock() : reward pays too much (actual=%d vs limit=%d)",
                block.vtx[0].GetValueOut(), GetBlockValue(pindex->pprev->nBits, pindex->pprev->nHeight)),
