...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeJadepayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeJadepayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeJadepayAmount = strAnonymizeJadepayAmount.remove(strAnonymizeJadepayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeJadepayAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeJadepayAmount * COIN) nMaxToAnonymize = nAnonymizeJadepayAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeJadepayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeJadepayAmount));
        strAnonymizeJadepayAmount = strAnonymizeJadepayAmount.remove(strAnonymizeJadepayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeJadepayAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
