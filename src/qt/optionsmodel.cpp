...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeJadepayAmount"))
        settings.setValue("nAnonymizeJadepayAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeJadepayAmount = settings.value("nAnonymizeJadepayAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeJadepayAmount"))
        SoftSetArg("-anonymizeJadepayamount", settings.value("nAnonymizeJadepayAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Jadepay.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeJadepayAmount:
            return QVariant(nAnonymizeJadepayAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeJadepayAmount:
            nAnonymizeJadepayAmount = value.toInt();
            settings.setValue("nAnonymizeJadepayAmount", nAnonymizeJadepayAmount);
            emit anonymizeJadepayAmountChanged(nAnonymizeJadepayAmount);
            break;
        case CoinControlFeatures:
